...
--
-- On Neon this runs through the pg_session_jwt extension: the auth provider
-- (Clerk, Auth0, or your own signer) issues a JWT, Neon validates it against
-- the provider's JWKS, and auth.user_id() returns the subject claim inside SQL.
-- The Supabase equivalent would be auth.uid().
--
--   CREATE EXTENSION IF NOT EXISTS pg_session_jwt;
--
-- Rules that must hold, because a cross-tenant leak ends the product:
--
--   1. RLS on EVERY table. Default deny — a new table with no policy returns
--      nothing, not everything.
--   2. farm_id is derived from the session, never accepted from the client.
--   3. The application connects as a role WITHOUT bypassrls. Migrations use a
--      separate owner role.
--   4. CI runs cross-tenant tests on every build: farm A's token must return
--      zero rows from every table belonging to farm B.
--
-- Helper, so policies stay readable and there is one place to change:
--
--   CREATE OR REPLACE FUNCTION current_farm_id() RETURNS uuid
--   LANGUAGE sql STABLE SECURITY DEFINER AS $fn$
--       SELECT farm_id FROM employee
--       WHERE auth_user_id = (auth.user_id())::uuid AND is_active
--       LIMIT 1;
--   $fn$;
--
--   ALTER TABLE rabbit ENABLE ROW LEVEL SECURITY;
--   ALTER TABLE rabbit FORCE ROW LEVEL SECURITY;
--
--   CREATE POLICY rabbit_tenant ON rabbit
--       USING      (farm_id = current_farm_id())
--       WITH CHECK (farm_id = current_farm_id());
--
-- WITH CHECK matters as much as USING: without it a caller can INSERT a row
-- stamped with someone else's farm_id.
--
-- Financial tables (invoice, subscription) additionally restrict to roles
-- ('owner', 'accountant'). Caretakers get UPDATE on their own recent records
-- only: created_by = self AND created_at > now() - interval '24 hours'
-- (the edit-window rule in docs/04-employee-module.md).
--
-- Enforce plan limits in the database too, not only in the UI — an over-limit
-- farm must be blocked from adding the NEXT doe, never have existing animals
-- hidden or deleted.
-- ============================================================================
